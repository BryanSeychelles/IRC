...
module.exports = {
	...
	if (!request.session.username || !request.session.current) {
	    response.send();
	    return ;
	}
		...
		await ircdata.find().toArray(function(error, docs) {
		    ...
		    deps.io.emit("chatodesu", "Channels: " + chans.join(" "));
		    response.json(docs);
		});
		client.close();
	    });
		...
		deps.mongo.connect(deps.url, {useUnifiedTopology: true}, async function(error, client) {
			...
			deps.io.emit("chatodesu", "Users in channel: " + docs[0].members.toString());
		    });
		    client.close();
		});
	    }
